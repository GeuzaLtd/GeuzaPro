...
export default function EditProductPage({ params }: { params: { id: string } }) {
  const existing = PRODUCTS_DATA.find((p) => p.id === parseInt(params.id));

  const [form, setForm] = useState({
    name:        existing?.name        ?? '',
    category:    existing?.category    ?? 'Wheelchair',
    price:       existing?.price?.replace('RWF ', '').replace(',', '') ?? '',
    ...
    <>
      ...
      <main className="flex-1 overflow-y-auto bg-gray-50">
        ...
        <div className="max-w-6xl mx-auto px-6 py-8">
          <PageHeader
            title={existing?.name ?? 'Edit Product'}
            description={`SKU: ${existing?.sku ?? params.id}`}
            breadcrumbs={[
