      ...
      >
        ...
        <div className="px-6 py-4 border-t border-gray-100 flex items-center justify-between flex-shrink-0">
          <button
            ...
            disabled={deleting}
            className="inline-flex items-center gap-2 px-4 py-2 rounded-full border border-red-100 text-sm font-medium text-red-400 hover:bg-red-50 transition-all disabled:opacity-60"
          >
            <HiOutlineTrash size={14} /> {deleting ? 'Deleting…' : 'Delete'}
          </button>
          {!sent && (
...
export default function MessagesView({ initialData }: { initialData: MessageRow[] }) {
  const router = useRouter();
  const [messages, setMessages] = useState<MessageRow[]>(initialData);
  ...
  const openMsg = (msg: MessageRow) => {
    if (msg.status === 'Unread') {
      setMessages((m) => m.map((x) => x.id === msg.id ? { ...x, status: 'Read' } : x));
      markMessageRead(msg.id).then(() => router.refresh());
    }
    setSelected(msg);
  };
