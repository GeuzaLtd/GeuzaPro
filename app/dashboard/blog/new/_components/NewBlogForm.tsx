...
export default function NewBlogForm({
  ...
  const setGallerySlot = (i: number) => (url: string | null) =>
    setGallery((prev) => prev.map((v, idx) => (idx === i ? url : v)));
            ...
            <div className="lg:col-span-2 flex flex-col gap-5">
              ...
              <motion.div custom={1} variants={fieldVariants} initial="hidden" animate="visible">
                <div className="bg-white rounded-2xl border border-gray-100 p-6">
                  ...
                  <ImageUploadSlot value={coverImage} onChange={(url) => setCoverImage(url)} isPrimary size="lg" folder="geuza/blog" />
                </div>
              </motion.div>
              ...
              <motion.div custom={3} variants={fieldVariants} initial="hidden" animate="visible">
                <div>
                  ...
                  <BlogEditor content={content} onChange={setContent} placeholder="Start writing your article here..." />
                </div>
              </motion.div>
              ...
              <motion.div custom={4} variants={fieldVariants} initial="hidden" animate="visible">
                <div className="bg-white rounded-2xl border border-gray-100 p-6">
                  <label className="block text-xs font-bold text-gray-400 uppercase tracking-widest mb-1">Other Images</label>
                  <p className="text-xs text-gray-400 mb-4">Additional photos for this post (not embedded in the editor)</p>
                  <div className="grid grid-cols-3 gap-3">
                    {gallery.map((url, i) => (
                      <ImageUploadSlot key={i} value={url} onChange={setGallerySlot(i)} size="sm" folder="geuza/blog" />
                    ))}
                  </div>
                </div>
              </motion.div>
              ...
              <motion.div custom={6} variants={fieldVariants} initial="hidden" animate="visible">
                <div className="bg-white rounded-2xl border border-gray-100 p-5">
                  ...
                  {allCategories.length === 0 ? (
                    <p className="text-xs text-gray-400 italic">No blog categories found</p>
                  ) : (
                    <div className="flex flex-col gap-1.5">
                      {allCategories.map((c) => (
                        <label key={c.id} className="flex items-center gap-3 cursor-pointer group py-1"
                          onClick={() => setForm({ ...form, categoryId: c.id })}>
                          <div className={`w-4 h-4 rounded border-2 flex items-center justify-center transition-all flex-shrink-0 ${form.categoryId === c.id ? 'bg-[#0F9E59] border-[#0F9E59]' : 'border-gray-300 group-hover:border-[#0F9E59]/50'}`}>
                          ...
                          <span className={`text-sm transition-colors ${form.categoryId === c.id ? 'text-gray-900 font-semibold' : 'text-gray-500 group-hover:text-gray-700'}`}>{c.name}</span>
                        </label>
                      ))}
                    </div>
                  )}
